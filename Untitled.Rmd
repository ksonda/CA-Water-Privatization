...
shutoffs_SF <- shutoffs %>% filter(QuestionName=="WR SHUT_OFFS Once SF Total") %>% mutate(sf_shutoffs=QuestionResults)%>%select(PWSID,sf_shutoffs) %>%distinct(.keep_all=TRUE)
shutoffs_MF <- shutoffs %>% filter(QuestionName=="WR SHUT_OFFS Once MF Total") %>% mutate(mf_shutoffs=QuestionResults)%>%select(PWSID,mf_shutoffs)%>%distinct(.keep_all=TRUE)
shutoffs_total <- pws_shutoff_collect %>% left_join(shutoffs_SF,by="PWSID")  %>% left_join(shutoffs_MF,by="PWSID") %>% mutate(shutoffs_total=as.numeric(sf_shutoffs)+as.numeric(mf_shutoffs)) %>% select(PWSID,shutoffs_total)

shutoff_comments <- shutoffs %>% filter(QuestionName=="WR SHUT_OFFS Comments")
  
shutoff_pa<- ear%>%filter(QuestionName=="WR SHUT_OFFS Payment Assistance Total")
```
